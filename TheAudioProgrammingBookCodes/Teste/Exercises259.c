/***********************************************************************************
    ...
    Description: Exercise from "The Audio Programming Book", chapter 2, exercises 2.5.9.
    Date:        23/06/2022
************************************************************************************/
...
int main(int argc, char** argv) {
    ...
    /* tremolo oscilator allocation */
    if ( (p_trem = new_oscil(outprops.srate)) == NULL ) {
        puts("No memory for tremolo oscillator\n");
        error++;
        goto exit;
    }
    ...
    for ( int i = 0; i < nbufs; ++i ) {
	    ...
	    for ( int j = 0; j < nframes * chans; ++j ) {
            ...
            outframe[j] = (float) (amp * tick(p_osc, freq) * sinetick(p_trem, 0.1));
        }
