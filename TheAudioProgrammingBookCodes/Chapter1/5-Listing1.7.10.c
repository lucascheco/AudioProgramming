...
int main(int argc, char* argv[])
{
    ...
    if (ismidi)
    {
        if (startval > 127.0)
        {
            printf("error: MIDI startval must be <= 127.0\n");
            return 1;
        }
        /* for MIDI startval = 0 is legal */
        if (startval < 0.0)
        {
            printf("error: MIDI starval must be >= 0.\n");
            return 1;
        }
    }
    else
    ...
    if (err < 0)
        printf("There was an error writing the file.\n");
